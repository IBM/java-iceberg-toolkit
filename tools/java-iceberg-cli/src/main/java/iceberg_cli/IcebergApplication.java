...
public class IcebergApplication {
    ...
    public String processRequest( String[] args ) throws Exception
    {
        ...
        MetastoreConnector connector = null;
        try {
            // Load catalog information
            customCatalog = new ConfigLoader().init(catalog, uri, warehouse);
            connector = CatalogUtils.getConnector(customCatalog, tableFormat, namespace, tableName, creds);
            ...
            case "create":
                if (tableName != null) {
                    ...
                    if (warehouse == null) {
                        ...
                        customCatalog.updateUriAndWarehouse(uri, warehouse);
                        // We need to re-initialize the connector
                        connector.close();
                        connector = CatalogUtils.getConnector(customCatalog, tableFormat, namespace, tableName, creds);
                    }
            ...
            if (customCatalog != null)
                AuthProvider.clearSession(customCatalog);
            if (connector != null)
                connector.close();
        }
    }
